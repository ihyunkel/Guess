...
function showTeamsScreen() {
    ...
    function renderTeams() {
                        ...
                        <div id="blueTeamContainer" style="display: flex; flex-direction: column; gap: 0.5rem">
                            ${gameState.teams.blue.map(player => `
                                <div class="team-player-item" data-player="${player}" data-team="blue" style="display: flex; align-items: center; justify-content: space-between; padding: 0.75rem; background: rgba(59, 130, 246, 0.2); border-radius: 8px; font-weight: 600">
                                    <span>${player}</span>
                                    <button class="move-player-btn move-to-red" data-player="${player}" style="padding: 0.25rem 0.75rem; background: #ef4444; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 0.9rem; transition: all 0.2s">
                                        ➡️
                                    </button>
                                </div>
                            `).join('')}
                        ...
                        <div id="redTeamContainer" style="display: flex; flex-direction: column; gap: 0.5rem">
                            ${gameState.teams.red.map(player => `
                                <div class="team-player-item" data-player="${player}" data-team="red" style="display: flex; align-items: center; justify-content: space-between; padding: 0.75rem; background: rgba(239, 68, 68, 0.2); border-radius: 8px; font-weight: 600">
                                    <button class="move-player-btn move-to-blue" data-player="${player}" style="padding: 0.25rem 0.75rem; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 0.9rem; transition: all 0.2s">
                                        ⬅️
                                    </button>
                                    <span>${player}</span>
                                </div>
        ...
        // Move to red buttons
        document.querySelectorAll('.move-to-red').forEach(btn => {
            btn.addEventListener('click', () => {
                const player = btn.getAttribute('data-player');
                movePlayerToRed(player);
            });
        });
        
        // Move to blue buttons
        document.querySelectorAll('.move-to-blue').forEach(btn => {
            btn.addEventListener('click', () => {
                const player = btn.getAttribute('data-player');
                movePlayerToBlue(player);
            });
        });
        ...
        // Continue button
        const continueBtn = document.getElementById('continueToSecretBtn');
        if (continueBtn) {
